...
This block captures the complete plan agreed with Wez on 17 Apr 2026. After the seed-files-and-dropdown delivery, the next session picks up from item 1 and works top-to-bottom.
...
### Build Order — 8 Deliveries, One At A Time

1. **Catalog limit raise 50 → 100 (Trial + Solo).** Files: `server/services/stripe.ts` (TIER_CONFIG.trial.maxCatalogItems, TIER_CONFIG.solo.maxCatalogItems, canAddCatalogItem default at line 776, hardcoded fallbacks at lines 556 and 638); `server/services/subscriptionRouter.ts` (line 109 hardcoded); `server/services/adminRouter.ts` (lines 123, 200 hardcoded); `drizzle/schema.ts` (organizations.maxCatalogItems.default from 50 to 100); `client/src/pages/Pricing.tsx` (line 390 "up to 50 items" → "up to 100 items"); `client/src/pages/Dashboard.tsx` (line 553 "50 catalog items" → "100 catalog items"). After deploy run on Render shell: `UPDATE organizations SET max_catalog_items = 100 WHERE max_catalog_items = 50;` — retroactively updates ALL existing Trial/Solo users immediately (Wez confirmed).

...
| 16 Mar 2026 | `server/r2Storage.ts`, `server/routers.ts` | **Logo signed URL expiry bug fixed.** R2 signed URLs default to 7 days (`X-Amz-Expires=604800`). Logo URLs stored in DB were expiring silently — logo appeared after upload but broke 7 days later. Root cause: `uploadToR2` always used 7-day expiry with no override. Fix: added `SIGNED_URL_LOGO_EXPIRY = 315360000` (10 years) constant and `longLived: boolean` param to `uploadToR2`. `uploadLogo` mutation in `routers.ts` now passes `longLived: true`. All other R2 uploads (quote inputs, PDFs) unaffected — still use 7-day default. Existing stored logo URLs in DB still have the old expired URLs — users must re-upload their logo once after this deploy to get a fresh 10-year URL. |

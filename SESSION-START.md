...
| 26 Mar 2026 | `drizzle/schema.ts` | **costPrice missing from quote_line_items table — margin never persisted after generateDraft.** Root cause: `costPrice` column existed on `catalogItems` table but was never added to `quoteLineItems` table in `drizzle/schema.ts`. The 14 Mar 2026 fix correctly wired `costPrice` through the `generateDraft` → `qdsLineItems` → `createLineItem` call chain, but Drizzle silently dropped the value at write time because the column wasn't in the schema mapping. Result: after generating a quote, all margin cells showed `—` and the margin totals were blank even when buy-in costs had been entered in QDS. Fix: added `costPrice: decimal("cost_price", { precision: 12, scale: 2 })` to `quoteLineItems` table definition. **Column added via raw SQL on Render shell:** `psql $DATABASE_URL -c "ALTER TABLE quote_line_items ADD COLUMN IF NOT EXISTS cost_price DECIMAL(12,2);"` — drizzle-kit push was aborted as it detected unrelated destructive changes. Electrical tables (`electricalTakeoffs`, `containmentTakeoffs`, `tenderContexts`) untouched. No other code changes needed — the read path in `QuoteWorkspace.tsx` already handles `item.costPrice` correctly. |
| 26 Mar 2026 | `client/src/pages/QuoteWorkspace.tsx` | **Margin summary breakdown by pricing type.** Previously the margin bar showed a single combined "Total Margin: £X (Y%)" figure that mixed one-off, monthly, annual, and optional margins together — misleading when a quote has both one-off hardware sales and recurring monthly tariffs. Fix: replaced the single-row IIFE with a multi-row breakdown. Each pricing type (standard, monthly, annual, optional) gets its own colour-coded row matching the existing revenue totals colour scheme (green for one-off, teal for monthly, amber for annual, purple for optional) — only shown when that type has at least one priced item with a known cost. Overall total row preserved below. `resolveCostPrice` helper extracted to avoid duplicating catalog-fallback logic. No other state, no server changes, no electrical code touched. Internal only — not on PDF. |
| 17 Apr 2026 | `server/engines/generalEngine.ts` | **IT sector — invoice/contract/statement evidence now extracted as quote scope (MSP workflow fix).** Root cause: the GeneralEngine system prompt was framed entirely around "what work is being requested" — emails, briefs, voice notes, specs. When an MSP user uploaded invoices or contracts from a previous provider (a core MSP workflow: prospect hands over incumbent's invoice to be matched/improved, renewal cycle, takeover/transition, cost-reduction review), Claude reasoned correctly within that brief and returned `materials: []` with a notes field saying "these are existing invoices, not a request for quotation". `jobDescription` was populated accurately so the `parseDictationSummary` empty-summary guard didn't fire — the QDS rendered with description + notes only, no line items, no labour. Reproduced by Wez with two Urban Network UK invoice JPGs (Bradnam Joinery client: M365 licences, BCDR, Sophos firewall, IT support contracts, dated engineer labour) — QDS showed description only, zero materials extracted. Fix: added a new `itInvoiceAddendum` const inside `GeneralEngine.analyse()` that evaluates to a dense IT-specific prompt block when `this.tradePreset === "it_services"` and to `""` for every other sector. Block is spliced into the existing systemPrompt via one `${itInvoiceAddendum}` interpolation placed immediately before "Respond ONLY with valid JSON" — so it lands as context rather than trailing noise. Block covers: (1) invoice/contract/statement as legitimate scope evidence (not a request-for-work document); (2) line-item mapping rules (strip "Contract:"/"Service:" prefixes, preserve `[NCE/1-Year/Monthly]`, `[36-month]`, `[Core]`, `[Gold]`, `/Baseline /Backup /Spam` tags in descriptions); (3) pricingType from cadence markers (monthly/annual mapping, each dated labour row as separate material, prorated catch-up lines dropped as historical noise); (4) UK MSP resale pricing ranges for redacted prices (M365 tiers, managed firewall, BCDR, managed server, engineer hourly rates, named-user support) — must pick a specific number and set `estimated: true`; (5) additive behaviour for mixed evidence (invoice + separate email request for additional work → extract both); (6) "don't invent scope" reinforcement. **Isolation:** gated entirely by `this.tradePreset === "it_services"` — for `commercial_cleaning`, `building_maintenance`, `pest_control`, `scaffolding`, `painting`, `custom`, and null/unrecognised sectors the interpolation evaluates to empty string and systemPrompt is byte-identical to pre-fix. DrawingEngine (17 sectors) and ElectricalEngine (1 sector) route through different engine files, physically cannot be reached by this change. `EngineOutput` contract unchanged. No schema changes, no router changes, no client changes, no PDF changes. `QuoteWorkspace.tsx`, `routers.ts`, `generateSimpleQuoteHTML` all untouched. Pre-existing `tsc --noEmit --skipLibCheck` errors (38, all in unrelated files) unchanged — zero new errors introduced. Action: Wez should re-run "Re-analyse" on quote 132 after deploy to regenerate the QDS with invoice line items extracted. |
| 17 Apr 2026 | `server/catalogSeeds/itServicesSeed.ts` (NEW), `server/catalogSeeds/index.ts` (NEW), `server/db.ts`, `server/routers.ts`, `server/engines/generalEngine.ts`, `client/src/pages/Catalog.tsx` | **IT Services starter catalog + smart category substitution.** Two complementary pieces. **(1) Starter catalog seeding.** New `server/catalogSeeds/` folder with `itServicesSeed.ts` (22 items: 10 Microsoft 365 & Licensing, 5 Security & Backup, 2 IT Support Contracts, 2 Website Services, 1 Productivity Tools, 2 Engineer Labour) and `index.ts` registry mapping `sector key → seed array`. All prices ex VAT, all items `pricingType: "monthly"` except labour (`"standard"`). Naming canonicalised to Microsoft's official product names so AI extraction from competitor invoices (e.g. `[NCE/1-Year/Monthly]` tags) matches cleanly. New DB helper `seedCatalogFromSectorTemplate(orgId, userId, sector)` in `server/db.ts` — reads registry, inserts rows with correct orgId/userId, returns `{seeded, sector}`. Auto-fires at end of `createUser()` inside the existing `if (user)` block, wrapped in try/catch so seeding failure never blocks registration. Existing users get a manual trigger: new `catalog.seedFromSectorTemplate` tRPC procedure (additive inside existing catalog router) + new "Load Starter Catalog" button on Catalog page empty state, gated by `SEEDABLE_SECTORS = ["it_services"]` check against `user.defaultTradeSector`. Button guarded by server-side check that catalog is empty (prevents duplicate seeding). **(2) Smart category substitution.** Extended the existing `itInvoiceAddendum` in `generalEngine.ts` with a SMART CATEGORY SUBSTITUTION block. Tells the AI that an MSP's catalog represents their chosen vendor in each commodity category — when invoice evidence names a different vendor in the same category (LastPass vs Keeper; Sophos Intercept X vs ESET; Mimecast vs Barracuda), substitute the user's catalog item at the user's catalog price. Explicit brand-name lists for 9 substitutable commodity categories (password managers, endpoint security, email threat protection, M365 backup, server BCDR, signature management, RMM, documentation, DNS filtering). Explicit no-substitute list for client-specific choices (productivity suites, firewall brands, hardware SKUs, telephony). Instruction to always prepend "Replaces existing [original product]" to the description — silent substitutions flagged as bugs. **Isolation:** all changes fire only when `sector === "it_services"`. Seed registry is empty for every other sector (returns null → seeding no-ops). Substitution block is inside the already-`it_services`-gated addendum. `QuoteWorkspace.tsx` not touched. `generateSimpleQuoteHTML` not touched. `routers.ts` changes are strictly additive (new import + new procedure; existing procedures byte-identical). `createUser()` in `db.ts` gets one new try-wrapped call — behaviourally invisible for any sector without a seed template. No schema changes, no migrations, no env var changes. Pre-existing `tsc --noEmit --skipLibCheck` error count identical (83 before, 83 after — only line numbers shifted by insertions). Zero new errors introduced in any file. Action for existing IT users (incl. Wez @ thegreenagents): visit Catalog page — if empty, click "Load Starter Catalog" button. Then test the Bradnam quote 132 (Urban Network invoice) by clicking Re-analyse — "Reach IT Support [Core] Named User" should now substitute to "Silver IT Support — Unlimited Remote" at £18/user/month with "Replaces existing Reach IT Support" in the description. |
| 17 Apr 2026 | `server/routers.ts`, `client/src/pages/Catalog.tsx` | **Starter catalog — relaxed empty-catalog gate (follow-up patch).** Original `catalog.seedFromSectorTemplate` refused to seed if the catalog already contained any items. This blocked legitimate scenarios: existing users with legacy catalog items from a different sector/product context wanted the IT starter kit added alongside their existing items. Fix: removed the `existing.length > 0` hard throw. Replaced with case-insensitive name-based dedupe — the procedure now loads the existing catalog, filters the seed to only items whose names aren't already present, and inserts the filtered set. Returns `{ seeded, skipped, sector }`. Idempotent — clicking twice is safe. Client side: new "Load Starter Catalog" button added to the Catalog page header next to "Add Item", shown whenever user's `defaultTradeSector` is in `SEEDABLE_SECTORS` (currently just `it_services`), regardless of whether catalog is empty. The existing empty-state teal box button is preserved and now shares a `handleSeedClick` helper that shows a confirm dialog when catalog has existing items ("This will add up to 22 IT Services starter items. Items with matching names will be skipped. Continue?"). Success toast adapts to seeded+skipped counts: all-new, all-skipped, and mixed all get distinct messages. Imports: `getCatalogSeedForSector` now imported directly from `./catalogSeeds` in `routers.ts` since the procedure builds its own insert loop rather than calling the `db.ts` helper (needed access to the raw seed array for the filter). The `seedCatalogFromSectorTemplate` helper in `db.ts` is still used by the auto-seed in `createUser` — untouched. No schema changes. Pre-existing `tsc --noEmit --skipLibCheck` error count identical (83 before, 83 after — only line numbers shifted by the one-line import addition). Zero new errors. Action for Wez: redeploy, then visit `/catalog`, click "Load Starter Catalog" button (now visible alongside existing 5 Marketing Campaigns items). 22 IT items will be added; existing items preserved. |
| 17 Apr 2026 | `server/routers.ts`, `client/src/pages/Catalog.tsx` | **Starter catalog — per-item selection dialog.** Previous one-click "Load Starter Catalog" was too blunt — users had no visibility into what would be added and no ability to cherry-pick. Replaced with a proper selection dialog: grouped checkbox picker, all available items selected by default, items already in the user's catalog shown but disabled with "Already added" badge, per-category "Select all / Deselect all" toggle, live selected count, "Add N Selected" confirm button disabled when N=0. Server changes: (1) new `catalog.getSectorTemplate` query returns `{ sector, items, alreadyInCatalog }` — lazily fetched only when the dialog opens (`enabled: isSeedDialogOpen` on the tRPC query). Plain-data projection of the seed registry, no functions/symbols leaked to client. Checks the user's catalog to populate `alreadyInCatalog` case-insensitively so the UI can grey those rows. (2) `catalog.seedFromSectorTemplate` now accepts optional `{ selectedNames: z.array(z.string()).optional() }` input. If provided, seeds only items whose names match (case-insensitive); if omitted, seeds all — preserves backward-compat for the auto-seed path in `db.ts createUser()`. Server-side dedupe by existing catalog names runs AFTER selection filter, so clicks are idempotent. Client changes in Catalog.tsx: imported `Checkbox` from shadcn/ui, added three new pieces of state (`isSeedDialogOpen`, `selectedSeedNames: Set<string>`, `seedInitializedRef` one-shot). `useEffect` initialises the selection from `seedTemplate.items` minus `alreadyInCatalog` once per dialog-open (ref resets on close so next open gets fresh state). New `toggleSeedItem`/`toggleSeedCategory` helpers. `handleSeedClick` now just opens the dialog (no more `window.confirm`). `handleConfirmSeed` passes `Array.from(selectedSeedNames)` to the mutation. The mutation's `onSuccess` closes the dialog and shows a toast with seeded/skipped counts. Both the header button and the empty-state teal box trigger the same dialog. **Isolation:** all changes are additive or modify only code added in this session. No schema changes. No migrations. `QuoteWorkspace.tsx`, `generateSimpleQuoteHTML`, all other engines/sectors — untouched. Pre-existing `tsc --noEmit --skipLibCheck` error count identical (83 before, 83 after — only line numbers shifted by ~40 lines from the new getSectorTemplate procedure). Zero new errors. |
| 18 Apr 2026 | `client/src/pages/Dashboard.tsx`, `client/src/pages/Catalog.tsx` | **GTM Delivery 4 — Dashboard seed-catalogue nudge (top banner + empty-state card) with one-click auto-open on Catalog.** Closes the onboarding gap for the three new GTM sectors (Website & Digital Marketing, Commercial Cleaning, Pest Control): users who registered post-17 Apr have a seedable starter catalogue sitting one click away on `/catalog`, but with no surface anywhere in the app pointing them at it. The Dashboard nudge fixes that. **Surfaces (two, shared dismiss):** (1) Persistent card banner inserted between the page header and the Stats Cards grid — teal-styled (`border-teal-200 bg-teal-50`) to match the existing teal "Load Starter Catalog" empty-state box on `/catalog`, with a `Sparkles` icon, "Kick-start your catalogue" title, one-sentence UK-market-anchored explainer, a primary "Load Starter Catalogue" button that navigates to `/catalog?seed=1`, and an `X` dismiss button in the corner. (2) Supplementary card inside the "No quotes yet" empty state block (lines 297–307 in the pre-delivery file) — same teal styling, smaller outline-variant button, added beneath the existing "Create Quote" primary so fresh users see "create first quote" and "load starter catalogue" as complementary first-runs rather than competing. **Gate — all three must be true to render either surface:** `user.defaultTradeSector` is in the client-side `SEEDABLE_SECTORS = ["it_services", "website_marketing", "commercial_cleaning", "pest_control"]` (same constant as in `Catalog.tsx:163`, duplicated per existing "client can't import server code" comment); `catalogItems?.length === 0` from a new `trpc.catalog.list.useQuery()` call on the Dashboard (free — shares TanStack Query cache with the same query on `/catalog` so round-tripping between the two pages costs one fetch total); `!nudgeDismissed`. Users on the 21 hidden sectors (electrical, plumbing, etc.) see nothing new. Users who've already seeded the catalogue see nothing — the banner self-heals without manual cache invalidation because `catalogItems.length === 0` flips false after seeding. **Dismiss state — one shared key, per-user namespaced:** `localStorage` key `idyq-seedcatalog-nudge-dismissed:${user.id}`. Read on mount via `useEffect(..., [(user as any)?.id])` to wait for auth hydration, written by `handleDismissNudge` on X click. Single `nudgeDismissed` boolean state drives both the banner and the empty-state card — X on either hides both instantly and persists across reloads. `try/catch` around every LS access (private mode / quota / SSR) — failure degrades gracefully to in-memory-only dismiss. **Catalog page auto-open (the other half of the flow):** new `didAutoOpenSeedRef` one-shot ref plus a `useEffect(..., [user, canSeedStarterCatalog])` on mount. Reads `?seed=1` from `new URLSearchParams(window.location.search)` (mirrors the existing pattern at `Settings.tsx:207`), waits for `user` to hydrate before evaluating `canSeedStarterCatalog`, then calls `setIsSeedDialogOpen(true)` and strips the query param via `window.history.replaceState(null, "", "/catalog")` so reloads/copy-paste of the URL don't re-trigger the dialog. One-shot ref prevents the effect from re-firing if `canSeedStarterCatalog` re-renders. Non-seedable users who hit the URL directly get the param stripped and no dialog — intentional. **Icons:** `Sparkles` (matches the existing seed dialog at `Catalog.tsx:541` + the empty-state teal box at line 430) and `X` (new dismiss control) added to the Dashboard `lucide-react` imports. No other dependency changes. **Isolation:** pure UI-layer additive. Two files touched, both on the client. No schema, no DB, no server, no tRPC, no engine, no electrical code. `QuoteWorkspace.tsx`, `generateSimpleQuoteHTML`, `routers.ts`, all engines, all sector prompts, the three new sector seed files, the four GTM sectors' dropdown entries — all untouched. Existing `/catalog` seed dialog behaviour unchanged — the auto-open effect just toggles the same `isSeedDialogOpen` state the existing "Load Starter Catalog" buttons toggle, so the existing `seedInitializedRef` selection-init effect fires on open exactly as before. Existing "Load Starter Catalog" header button (line 354) and empty-state teal box (line 428) still work unmodified. Pre-existing `tsc --noEmit --skipLibCheck` output is **byte-identical — 83 before, 83 after**, diffed against the baseline sorted output (`diff` returned no rows). Zero new errors. **Post-deploy actions:** none. No SQL, no `drizzle-kit push`, no data migration. Dismiss state is per-browser (LS), so a user dismissing on desktop still sees the nudge on mobile — acceptable default; flag if you want to persist via an org/user column instead. **Test path:** (1) log in as a user on any of the four GTM sectors with an empty catalogue → Dashboard shows teal banner above Stats Cards; if no quotes exist, also shows the smaller teal card inside the empty state. (2) Click "Load Starter Catalogue" on either surface → routes to `/catalog?seed=1`, seed dialog auto-opens, URL becomes `/catalog`. (3) Cancel the dialog, navigate back to Dashboard via header → banner still shows (catalogue still empty, dismiss not clicked). (4) Confirm the dialog with items selected → dialog closes, catalogue populates; navigate back to Dashboard → banner gone (`catalogItems.length !== 0`). (5) Clear your catalogue in a test org, dismiss the banner with X → banner disappears, Dashboard refreshes confirm it stays gone. (6) Log in as a user on a hidden sector (e.g. `electrical` or `plumbing`) → no banner, no empty-state nudge, regardless of catalogue state. **Known cosmetic wrinkle:** a previously-dismissed user may see a single-frame flash of the banner on Dashboard mount before the LS read effect resolves — mitigated by React's batched render but theoretically visible on slow devices. Judged not worth the complexity of reading LS synchronously in the `useState` initialiser since LS access doesn't know the user ID until auth hydrates. |
| 18 Apr 2026 | `client/src/pages/Register.tsx` | **GTM Delivery 3 hotfix — Register.tsx was still importing `TRADE_SECTOR_OPTIONS` (all 26 sectors) in live production.** Wez deployed Delivery 3 and tested the Register page: all 26 sectors were still appearing in the dropdown (Structural Steel, Solar PV, Telecoms Cabling, Bespoke Metalwork, etc.) despite the Delivery 3 Changes Log row asserting Register.tsx already used `VISIBLE_TRADE_SECTOR_OPTIONS`. Investigation: the session-start zip Claude was working from had a version of Register.tsx using the visible list, but the actually-deployed file in `main` was still using the full list — a divergence between the working-copy zip and live main that Claude missed. The Delivery 3 row's assertion that "The 17 Apr deploy already introduced `VISIBLE_TRADE_SECTOR_OPTIONS` ... and wired `Register.tsx` + `Settings.tsx` to use it" was incorrect for Register.tsx — only `Settings.tsx` was actually wired. Wez supplied the genuinely-deployed Register.tsx and the fix is a single two-line edit: `import { TRADE_SECTOR_OPTIONS }` → `import { VISIBLE_TRADE_SECTOR_OPTIONS }` on line 10, and `{TRADE_SECTOR_OPTIONS.map(...)}` → `{VISIBLE_TRADE_SECTOR_OPTIONS.map(...)}` on line 172. **Isolation:** purely a UI-layer import + variable name swap inside Register.tsx. No schema, no server, no tRPC, no engine code, no other files. `QuoteWorkspace.tsx`, `generateSimpleQuoteHTML`, `routers.ts`, all engines, all electrical code — untouched. Engine routing unaffected (all 26 keys remain valid at engine level). Settings.tsx unaffected (already correct from Delivery 3 proper). `Dashboard.tsx` tradePreset dropdown unaffected (different data source — `TRADE_PRESETS` via tRPC query). Pre-existing `tsc --noEmit --skipLibCheck` error count identical: **83 before, 83 after** — zero new errors. **Post-deploy action:** none — just deploy and hard-refresh the Register page. The dropdown should now show exactly 5 entries: IT Services / MSP, Website & Digital Marketing, Commercial Cleaning, Pest Control, Other / Custom. **Lesson captured for future sessions:** before asserting in a Changes Log row that an existing file is already in a particular state, verify against the actually-deployed code by asking Wez to upload the live file, not by relying solely on the session-start zip. The zip provides recency bias but not ground truth if other sessions/agents have touched the file in between. |
| 18 Apr 2026 | `client/src/lib/tradeSectors.ts`, `client/src/pages/Settings.tsx` | **GTM Delivery 3 — sector visibility filter with hidden-value preservation.** The 17 Apr deploy introduced `VISIBLE_TRADE_SECTOR_OPTIONS` (the five GTM + custom entries) in `tradeSectors.ts` and wired `Settings.tsx` to use it, so on paper new users only see the narrowed list (but see the 18 Apr Delivery 3 hotfix row above — `Register.tsx` was actually still using the full list in live production; that row corrects the oversight). Delivery 3 closes the remaining gap: existing users whose `defaultTradeSector` is a hidden sector (e.g. `electrical`, `plumbing`, `roofing`, any of the 21 deprioritised keys) were previously shown the five-option list in Settings with no match for their current value. The shadcn `<Select>` binds its display to an exact `SelectItem` match — with no match, the trigger fell back to the `"Select your business sector..."` placeholder, making the user think their setting was blank. Any subsequent save would silently overwrite their previously chosen sector. **Fix:** new helper `getVisibleTradeSectorOptions(currentValue?: string | null)` exported from `tradeSectors.ts`. Returns the straight five-option visible list when `currentValue` is nullish, empty, already visible, or unrecognised. When `currentValue` is a valid but hidden sector key, it looks up that key's full label from the 26-entry `TRADE_SECTOR_OPTIONS` and prepends one extra entry to the list with the label annotated `" (current)"` — so an existing `electrical` user now sees `"Electrical Installation (current)"` at the top of their dropdown with their existing value bound, keeping their settings intact. They can still change to any of the five GTM options, or leave it — no silent overwrite. `Settings.tsx` swaps `VISIBLE_TRADE_SECTOR_OPTIONS.map(...)` for `getVisibleTradeSectorOptions(defaultTradeSector).map(...)` on its `<SelectContent>` render. Import trimmed accordingly — `VISIBLE_TRADE_SECTOR_OPTIONS` no longer imported in Settings (helper handles the list internally). `Register.tsx` not addressed in this row — see the 18 Apr Delivery 3 hotfix row for the separate fix that made Register use the visible list. **Engine routing untouched:** all 26 sector keys remain valid in `engineRouter.selectEngine()` — a user on a hidden sector who doesn't change it in Settings continues to route through their original engine (DrawingEngine for e.g. `plumbing`, ElectricalEngine for `electrical`) exactly as before. Legacy quotes and legacy user setups are fully preserved. **Isolation:** pure additive UI-layer change. No schema changes, no DB changes, no server changes, no tRPC changes, no engine changes, no electrical code touched. `QuoteWorkspace.tsx`, `generateSimpleQuoteHTML`, `routers.ts`, `Dashboard.tsx` tradePreset dropdown (which is driven by server-side `TRADE_PRESETS` via a tRPC query — different data source — also untouched). Pre-existing `tsc --noEmit --skipLibCheck` error count identical: **83 before, 83 after** — zero new errors, zero new errors in either touched file. **Post-deploy actions:** none. No SQL, no `drizzle-kit push`. Test: log in as a user whose `defaultTradeSector` is `electrical` (or set a test org that way on the Render shell), visit Settings → the dropdown should show "Electrical Installation (current)" as a selectable option alongside the five GTM entries. IT users and the three new GTM sectors see the unchanged five-option list. |
| 18 Apr 2026 | `server/routers.ts`, `client/src/pages/Catalog.tsx` | **GTM Delivery 2 — tier-aware seeding guard + SEEDABLE_SECTORS expansion.** The "Load Starter Catalog" feature now respects the user's plan cap and no longer silently lets users exceed their Trial/Solo 100-item limit. Also fixes a stale client-side hardcode that was blocking 3 of the 4 GTM sectors from reaching the seed dialog at all. **Server changes (two procedures inside `catalog` router — both added in the 17 Apr session, so modifications stay within the routers.ts add-only rule):** (1) `catalog.getSectorTemplate` now returns `{ sector, items, alreadyInCatalog, maxCatalogItems, currentCount, remaining }` — the three new fields let the client render live capacity headroom. `maxCatalogItems` mirrors `org.maxCatalogItems` with a `?? 100` fallback matching `canAddCatalogItem` in `stripe.ts`. `-1` passes through to mean unlimited (Pro/Team/Business). `currentCount` is `existing.length` from the already-fetched catalog (no extra DB hit — reuses the same `getCatalogItemsByOrgId` call that powers `alreadyInCatalog`). `remaining` is `-1` when unlimited, else `max(0, cap - currentCount)`. Empty-sector and no-seed branches also return the capacity fields so the UI has consistent data. (2) `catalog.seedFromSectorTemplate` gains a pre-flight cap check immediately before the insert loop: when `cap !== -1` and `existing.length + toSeed.length > cap`, throw a friendly error `"Adding N items would exceed your 100-item catalogue cap. Deselect K items or upgrade your plan for unlimited catalogue."` The check runs after selection filter + dedupe so `toSeed.length` is the true number of new rows that would be written. Defense-in-depth behind the client disable — protects against race conditions (user opens dialog, admin adds items elsewhere, user confirms) and against any future client that forgets the guard. **Stale error-string fix:** the no-seed-available error message in `seedFromSectorTemplate` previously said `"This feature is currently available for: IT Services."` — updated to list all four sectors with seeds: `"IT Services, Website & Digital Marketing, Commercial Cleaning, Pest Control."` **Client changes in `Catalog.tsx`:** (1) `SEEDABLE_SECTORS` expanded from `["it_services"]` to all four GTM sectors (`it_services`, `website_marketing`, `commercial_cleaning`, `pest_control`). **This was actively broken** — the 17 Apr deploy landed seed files + registry entries for the three new sectors, but this client hardcode meant users on `website_marketing`, `commercial_cleaning`, or `pest_control` never saw the "Load Starter Catalog" button, so the feature was unreachable for 3 of 4 GTM sectors since the deploy. Kept as a client-side constant per the existing comment — client can't import server code. (2) Seed-dialog footer now has a capacity subline under the "N items selected" count: when org has a finite cap it shows `"K of M catalogue slots free after this addition."` (where K is the post-selection remaining); when user selects more than fits it flips to amber `"Would exceed your M-item plan cap — deselect X items to proceed."` Unlimited plans get no subline (nothing useful to say). (3) Confirm button `disabled` condition gains `|| (seedTemplate.maxCatalogItems !== -1 && selectedSeedNames.size > seedTemplate.remaining)` so over-cap selections can't even submit. **Isolation:** every touched code path was added in the 17 Apr session (both procedures, the whole seed dialog). No existing procedures modified. No schema changes. No new database queries (capacity computed from an existing fetch). No engine code, no electrical code, no shared state changes. `QuoteWorkspace.tsx`, `generateSimpleQuoteHTML`, all engines, all electrical files, all sector prompts — untouched. Pricing tiers and `TIER_CONFIG` unchanged. Pre-existing `tsc --noEmit --skipLibCheck` error count identical: **83 before, 83 after** — zero new errors. Line-number shifts in two pre-existing `routers.ts` `costPrice` errors reflect the ~56 lines added to `getSectorTemplate` + pre-flight block. The two cosmetic field-order diffs in `QuoteWorkspace.tsx` and `pdfGenerator.ts` error renderings are the same non-deterministic compiler rendering we saw in Delivery 1 — not new errors, not shape changes. **Post-deploy actions:** none. No SQL, no `drizzle-kit push`, no data migration. Pure additive behaviour — users on the 4 GTM sectors can now reach and use the starter catalog with live capacity feedback. |
| 18 Apr 2026 | `drizzle/schema.ts`, `shared/schema.ts`, `server/services/stripe.ts`, `server/services/adminRouter.ts`, `server/services/subscriptionRouter.ts`, `client/src/pages/Pricing.tsx`, `client/src/pages/Dashboard.tsx` | **GTM Delivery 1 — catalog item limit raised 50 → 100 for Trial + Solo tiers.** Strategic rationale: the three new GTM sectors (Website & Digital Marketing, Commercial Cleaning, Pest Control) ship with starter catalogs in the 24–28 item range; combined with a user's own additions, 50 items was going to feel tight on lower tiers during onboarding. 100 gives genuine room without cannibalising the Pro+ "unlimited" differentiator. Paid tiers (Pro, Team, Business) unchanged at `-1` (unlimited). **Every hardcoded `50` in the catalog-limit path updated, 10 sites in 7 files.** `drizzle/schema.ts:47` — `organizations.maxCatalogItems` column `.default(50)` → `.default(100)` (authoritative schema; drizzle.config.ts points here). `shared/schema.ts:54` — matching default in the legacy/orphaned parallel schema file (imported only by `ElectricalWorkspace.tsx` for the `QuoteInput` type, so default value doesn't affect runtime, but updated for hygiene so future sessions don't get confused by mismatched defaults). `server/services/stripe.ts` — four changes: `TIER_CONFIG.trial.maxCatalogItems` line 23, `TIER_CONFIG.solo.maxCatalogItems` line 32, the hardcoded fallback inside the `customer.subscription.deleted` webhook handler at line 556 (when a sub is canceled the org reverts to trial-like state), and the `canAddCatalogItem` nullish fallback at line 776 `org.maxCatalogItems ?? 50 → ?? 100`. `server/services/adminRouter.ts` lines 123 + 200 — nullish fallbacks in the `listOrganizations` and `getOrganizationDetail` queries that enrich admin responses with `maxCatalogItems ?? 50`. `server/services/subscriptionRouter.ts:109` — the `tiers` public query (used by `Pricing.tsx` to render plan comparison) hardcoded `solo.maxCatalogItems: 50`. Client copy: `Pricing.tsx:390` "Basic product catalogue (up to 50 items)" → "up to 100 items"; `Dashboard.tsx:553` Solo plan description `"50 catalog items"` → `"100 catalog items"`. **Sites NOT requiring change** (auto-propagate from TIER_CONFIG): `stripe.ts:638` `activateSubscription` uses `config.maxCatalogItems` — reads from TIER_CONFIG so the trial + solo bumps flow through automatically on any future activation; `stripe.ts:798` `getUpgradeSuggestion` reads from `config.maxCatalogItems`; `subscriptionRouter.ts:84` reads from `config.maxCatalogItems`; `adminRouter.ts:296` reads from `config.maxCatalogItems`. These were verified to be tier-aware and correct by construction. **Isolation:** pure value change. No schema shape changes (column type still `integer`, name still `max_catalog_items`, still nullable). No type shape changes. No new procedures, no deleted procedures, no changed procedures. `QuoteWorkspace.tsx`, `generateSimpleQuoteHTML`, `routers.ts` — untouched. No engine code touched, no sector prompts touched, no electrical code touched. No new dependencies. All other tiers (Pro/Team/Business at `-1`) unchanged. Pre-existing `tsc --noEmit --skipLibCheck` error count **identical: 83 before, 83 after** — zero new errors. (Two pre-existing errors show minor cosmetic re-ordering of fields in TypeScript's rendered error messages — same error codes, same line numbers, same files. Value-level integer default changes cannot alter type shapes; confirmed as compiler display non-determinism.) **Required post-deploy action for Wez:** (1) run on Render shell: `UPDATE organizations SET max_catalog_items = 100 WHERE max_catalog_items = 50;` — this retroactively bumps every existing Trial/Solo org from 50 to 100 immediately (Wez confirmed in planning). (2) Optionally run `npx drizzle-kit push` on Render shell to sync the column default for any future new orgs — but the SQL UPDATE above handles both new + existing rows, so the push is belt-and-braces. No data migration needed, no downtime, no user-visible breakage possible. Users never go below their previous allowance — 50-capped users become 100-capped, no one loses items. |
| 17 Apr 2026 | `server/catalogSeeds/websiteMarketingSeed.ts` (NEW), `server/catalogSeeds/commercialCleaningSeed.ts` (NEW), `server/catalogSeeds/pestControlSeed.ts` (NEW), `server/catalogSeeds/index.ts`, `client/src/lib/tradeSectors.ts` | **GTM narrowing — three new sector starter catalogs + Website & Digital Marketing sector added.** Strategic decision: deprioritise the 17 DrawingEngine sectors and the electrical sector for initial go-to-market. Focus on four GTM sectors that all ride GeneralEngine cleanly and share the services-heavy / invoice-evidence-driven shape: IT Services (already shipped), Website & Digital Marketing (new sector, new seed), Commercial Cleaning (new seed), Pest Control (new seed). **Three new seed files** following the `IT_SERVICES_CATALOG_SEED` shape exactly — same `CatalogSeedItem` type, same `readonly` + `as const`, same ex-VAT pricing, same `||`-delimited description format for AI extraction. Item breakdowns: `WEBSITE_MARKETING_CATALOG_SEED` 28 items (7 categories: Web Design & Development, Hosting & Care Plans, SEO Services, Paid Media Management, Social Media Management, Content & Branding, Labour Rates; 10 monthly, 17 standard, 1 annual). `COMMERCIAL_CLEANING_CATALOG_SEED` 26 items (6 categories: Recurring Cleaning Contracts, Periodic Deep Cleans, Specialist Services, Washroom & Consumables, One-Off Callouts, Labour Rates; 10 monthly, 16 standard). `PEST_CONTROL_CATALOG_SEED` 24 items (6 categories: Commercial Pest Control Contracts, One-Off Treatments — Domestic, One-Off Treatments — Commercial, Proofing & Exclusion, Monitoring Systems, Labour & Callouts; 6 monthly, 18 standard). **Registry updated**: `server/catalogSeeds/index.ts` imports the three new seeds and registers them under `website_marketing`, `commercial_cleaning`, `pest_control` keys. Existing `IT_SERVICES_CATALOG_SEED` and its entries are **completely unchanged per user instruction** — Wez's existing IT prices/naming/categories are production-validated and are not to be altered. **Sector dropdown**: `TRADE_SECTOR_OPTIONS` in `client/src/lib/tradeSectors.ts` gains one new entry `{ value: "website_marketing", label: "Website & Digital Marketing" }` placed immediately after `it_services` so the four GTM sectors group visually. `commercial_cleaning` and `pest_control` were already in the dropdown — those seeds become reachable as soon as this deploy lands. **Design principle captured**: for the three new sectors where Wez doesn't have hands-on pricing experience, figures are UK mid-market reference anchors that every new user will edit. IT's catalog is the model (Wez's real numbers) but the other three are starter scaffolds. `costPrice` populated only where genuine buy-in cost exists (hosting infrastructure, consumables, monitoring hardware); most service-labour items are `costPrice: null` because margin is labour-absorbed. **Isolation**: pure additive. No existing file modified except the two listed (`index.ts` registry and `tradeSectors.ts` dropdown). No engine code touched. No router code touched. No schema changes. `QuoteWorkspace.tsx`, `generateSimpleQuoteHTML`, `routers.ts`, IT catalog — all byte-identical to before. Pre-existing `tsc --noEmit --skipLibCheck` error count unchanged. Zero new errors. **Action on deploy**: commercial_cleaning and pest_control users can immediately click "Load Starter Catalog"; new `website_marketing` registrants get auto-seed via `createUser() → seedCatalogFromSectorTemplate`. **Not yet wired** (next session): catalog limit raise Trial/Solo 50→100; tier-aware seeding guard; sector visibility filter (hide 21 non-GTM sectors); Dashboard nudge banner + empty-state card (dismissible); addendum blocks for the three new sectors in `generalEngine.ts` (mirror of `itInvoiceAddendum`); demo quote auto-seed Option A for all four GTM sectors; IT-specific comprehensive-quote proposal template in `generateDraft`. All eight items documented in Pending Work block below. |

---

## Pending Work — GTM Rollout (Carry Into Next Session)

This block captures the complete plan agreed with Wez on 17 Apr 2026. Deliveries 1–4 shipped 18 Apr. The next session picks up from item 5 and works top-to-bottom.

### Strategic Context

**GTM scope decision:** ship IdoYourQuotes to four sectors only — IT Services (`it_services`), Website & Digital Marketing (`website_marketing`), Commercial Cleaning (`commercial_cleaning`), Pest Control (`pest_control`). These four all ride GeneralEngine cleanly and share a services-heavy / invoice-evidence shape. All 17 DrawingEngine sectors and the Electrical sector are deprioritised for initial GTM (code remains; just hidden from new-user selection).

**IT catalog is locked** — Wez does NOT want existing IT prices, names, categories, or pricingType values altered. IT is production-validated. The three new sector catalogs are starter scaffolds users will edit — IT is real production data.

**Demo quote strategy:** Option A across all four GTM sectors. Each sector auto-seeds one canonical example quote (titled "Example — [sector-appropriate title]") into the new user's account on registration, mirroring the catalog auto-seed pattern.

### Build Order — 4 Remaining Deliveries, One At A Time (Deliveries 1–4 shipped 18 Apr)

1. ✅ **DONE 18 Apr 2026** — Catalog limit raise 50 → 100 (Trial + Solo). Code shipped. **Required post-deploy action:** run on Render shell: `UPDATE organizations SET max_catalog_items = 100 WHERE max_catalog_items = 50;` to retroactively bump all existing Trial/Solo orgs. See Changes Log 18 Apr entry for full detail. ✅ SQL executed 18 Apr (4 rows updated).

2. ✅ **DONE 18 Apr 2026** — Tier-aware seeding guard + SEEDABLE_SECTORS expansion to all four GTM sectors. `getSectorTemplate` now returns capacity (`maxCatalogItems`/`currentCount`/`remaining`); `seedFromSectorTemplate` enforces the cap server-side; Catalog.tsx surfaces capacity hints and disables over-cap submits. Also fixes the stale `SEEDABLE_SECTORS = ["it_services"]` client hardcode that was blocking the starter-catalog button from appearing for users on the three new GTM sectors. No post-deploy action required. See Changes Log 18 Apr entry for full detail.

3. ✅ **DONE 18 Apr 2026** — Sector visibility filter with hidden-value preservation + Register.tsx hotfix. New `getVisibleTradeSectorOptions(currentValue)` helper in `tradeSectors.ts` prepends a hidden `currentValue` to the visible list with `" (current)"` suffix, so existing users on deprioritised sectors (electrical, plumbing, etc) see and keep their current value in Settings. **Hotfix**: Register.tsx was found in live production still importing `TRADE_SECTOR_OPTIONS` (all 26 sectors) despite the session-start zip showing it using the visible list — fixed with a two-line import + `.map` swap. Engine routing untouched — all 26 keys still valid. No post-deploy action required beyond redeploy + hard refresh. See Changes Log 18 Apr entries for full detail (two rows — Delivery 3 proper, and Delivery 3 hotfix).

4. ✅ **DONE 18 Apr 2026** — Dashboard nudge banner + empty-state card with auto-open on `/catalog`. Dismissible teal card banner inserted between the page header and the Stats Cards grid on Dashboard; supplementary smaller teal card added inside the "No quotes yet" empty state beneath the existing "Create Quote" button. Both gated on `user.defaultTradeSector ∈ SEEDABLE_SECTORS` (the four GTM keys, hardcoded client-side) AND `catalogItems?.length === 0` (from a new `trpc.catalog.list.useQuery()` on Dashboard, cache-shared with `/catalog`) AND `!nudgeDismissed`. Single shared dismiss state, per-user namespaced `localStorage` key `idyq-seedcatalog-nudge-dismissed:${user.id}` — X on either surface dismisses both instantly and across reloads. Both surfaces navigate to `/catalog?seed=1`; new auto-open `useEffect` in Catalog.tsx reads the param on mount, waits for auth hydration, opens the existing seed dialog, then strips the param via `history.replaceState`. One-shot ref prevents re-firing. Zero server changes, zero schema changes, `QuoteWorkspace.tsx` / `generateSimpleQuoteHTML` / `routers.ts` / all engines untouched. No post-deploy action required. See Changes Log 18 Apr entry for full detail.

5. **Addendum blocks for the three new sectors** in `server/engines/generalEngine.ts`. Mirror the existing `itInvoiceAddendum` pattern: `const websiteMarketingAddendum = this.tradePreset === "website_marketing" ? \`...\` : ""` (similar for `commercial_cleaning`, `pest_control`). Each addendum covers: (a) invoice/contract/statement as legitimate scope evidence for that sector; (b) line-item mapping rules and canonical naming per sector; (c) pricingType cadence markers (monthly retainer vs project vs annual); (d) UK mid-market price anchors for redacted prices in that sector; (e) additive behaviour for mixed evidence; (f) smart substitution where applicable (e.g. website hosting vendors are substitutable, Google Ads / Meta management fees are not — they're platform-locked). Splice all three alongside `${itInvoiceAddendum}` in the systemPrompt. Isolation rule holds — each gated by `this.tradePreset === "<sector>"` so non-target sectors get empty string.

6. **Demo quote auto-seed (Option A across all four GTM sectors).** New folder `server/demoQuotes/` with four files: `itServicesDemo.ts`, `websiteMarketingDemo.ts`, `commercialCleaningDemo.ts`, `pestControlDemo.ts` (each exporting a `getDemoQuote()` factory that returns `{ quoteFields, qdsSummaryJson, lineItems[] }` using names from the matching sector's catalog so catalog prices flow through). New `server/demoQuotes/index.ts` registry mapping sector key → factory, mirroring the `catalogSeeds/index.ts` pattern. New `seedDemoQuoteForSector(orgId, userId, sector)` helper in `server/db.ts` — try-wrapped, calls `createQuote` + loops `createLineItem`. Add one call in `createUser()` after the existing catalog seed call at line 264, wrapped in same try/catch. Add additive tRPC procedure `quotes.seedDemoForSector` (or extend existing quotes router with a new procedure — both are add-only-compatible) so existing users can trigger it manually — expose via a "Load Example Quote" button tied into the nudge card from delivery 4. All demo quotes use `status: "draft"` and include "(Example)" in the title.

7. **IT comprehensive-quote proposal template.** Inside `ai.generateDraft` in `server/routers.ts` at roughly line 4030+ (the `if (isComprehensive)` block starting at line 4032). Add a branch: when `tradePresetKey === "it_services" && isComprehensive`, use an IT-specific `systemPrompt` with MSP-appropriate JSON shape — Services Overview framing (not "Executive Summary of construction project"), line items grouped via `phase` field ("One-off Setup", "Monthly Recurring", "Annual Licences"), `timeline.phases` renamed as service-delivery milestones (onboarding / ongoing / reviews), IT-specific `assumptions` prompts (user count, site count, business-hours vs 24/7 support, included licence seats), `qualityCompliance` using Cyber Essentials / ISO 27001 / GDPR language, lighter `siteRequirements` (remote support defaults), drop construction-specific `technicalReview` or repurpose as "Security Review". Wez pre-approved this conditional branch inside `generateDraft` as a permitted exception to the "routers.ts add-only" rule. **CRITICAL pre-code step**: before writing the IT schema, read `generateComprehensiveQuoteHTML` in `server/pdfGenerator.ts` to verify how the PDF renderer handles missing/different fields — the IT schema must either preserve the shape of the existing schema with IT-appropriate content, OR the PDF renderer must gracefully handle nullable/alternative fields. Do not change `generateSimpleQuoteHTML`.

8. **Addendum blocks polish round (optional, after items 1–7 shipped).** Revisit the three new sector addendum blocks using ChatGPT research outputs Wez plans to run externally. Research prompts for all four GTM sectors were drafted in the 17 Apr session but Wez decided overkill for now and declined. If Wez later runs them and shares outputs, use findings to: (a) add/remove/rename seed catalog items; (b) refine addendum pricing anchors; (c) refine canonical naming; (d) refine substitution rules. IT catalog remains untouched — research is additive only.

### Deferred / Not In Scope

- Electrical sector further work (off-limits; defer indefinitely per GTM narrowing)
- DrawingEngine Phase 5 prompt injections (Bug 8 in Known Bugs) — deferred
- Pre-existing ~83 tsc errors in AdminPanel/stripe.ts etc — ignore unless directly relevant
- Bug 10 (Wez's monthlyQuoteCount reset) — harmless, will auto-reset on next Stripe billing tick

### Open Question Deferred To Next Session

Item 7's IT comprehensive template: whether to use the exact same JSON schema shape as the existing comprehensive prompt (just IT-appropriate wording) or introduce a new alternative shape. Answer depends on reading `generateComprehensiveQuoteHTML` first. Do not commit to a schema design before that read.

...
This block captures the complete plan agreed with Wez on 17 Apr 2026. Deliveries 1–3 shipped 18 Apr. The next session picks up from item 4 and works top-to-bottom.
...
### Build Order — 5 Remaining Deliveries, One At A Time (Deliveries 1–3 shipped 18 Apr)
...
4. **Dashboard nudge.** `client/src/pages/Dashboard.tsx`: add dismissible banner at top of dashboard (above tier card grid) AND enhance "No quotes yet" empty state (currently at line 296–307) with a seed-catalog nudge card. Condition: `user.defaultTradeSector` is in `SEEDABLE_SECTORS` AND `catalogItems.length === 0` AND not dismissed. **Single shared dismiss state** via `localStorage` key `idyq-seedcatalog-nudge-dismissed:${userId}` (Wez confirmed shared, not separate). Both nudge surfaces link to `/catalog?seed=1`. Update `client/src/pages/Catalog.tsx` to read the URL param on mount and auto-open the existing seed selection dialog.

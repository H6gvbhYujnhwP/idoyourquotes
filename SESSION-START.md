...
3. ✅ **DONE 18 Apr 2026** — Sector visibility filter with hidden-value preservation. New `getVisibleTradeSectorOptions(currentValue)` helper in `tradeSectors.ts` prepends a hidden `currentValue` to the visible list with `" (current)"` suffix, so existing users on deprioritised sectors (electrical, plumbing, etc) see and keep their current value in Settings instead of getting a silent overwrite. `Register.tsx` unchanged (new users have no prior value). Engine routing untouched — all 26 keys still valid. No post-deploy action required. See Changes Log 18 Apr entry for full detail.

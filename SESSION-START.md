...
This block captures the complete plan agreed with Wez on 17 Apr 2026. Delivery 1 shipped 18 Apr. The next session picks up from item 2 and works top-to-bottom.
...
### Build Order — 7 Remaining Deliveries, One At A Time (Delivery 1 shipped 18 Apr)

1. ✅ **DONE 18 Apr 2026** — Catalog limit raise 50 → 100 (Trial + Solo). Code shipped. **Required post-deploy action:** run on Render shell: `UPDATE organizations SET max_catalog_items = 100 WHERE max_catalog_items = 50;` to retroactively bump all existing Trial/Solo orgs. See Changes Log 18 Apr entry for full detail.

2. **Tier-aware seeding guard.** Modify `catalog.getSectorTemplate` in `server/routers.ts` to additionally return `{ maxCatalogItems, currentCount, remaining }` in the response shape. Modify `catalog.seedFromSectorTemplate` to pre-flight-check `existing.length + toSeed.length` against `org.maxCatalogItems` — throw friendly error if would exceed. Both are procedures added in recent sessions so modifying them is within the add-only rule for routers. Update `client/src/pages/Catalog.tsx` seed dialog to surface remaining capacity and disable confirm button when selection would exceed cap.

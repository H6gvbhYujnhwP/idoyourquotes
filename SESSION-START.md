...
| # | Issue | Location | Priority |
|---|---|---|---|
| C1 | **Measurement accuracy** — lengths calculated from annotation label spacing, not actual line geometry. Results are unreliable (100mm LV: 240 lengths measured vs 130 lengths actual). Root fix: extract vector line geometry from PDF (x1,y1,x2,y2 per segment) and sum line lengths grouped by proximity to size annotations. | `server/services/electricalTakeoff.ts` (`extractPdfLineColours` return type), `server/services/containmentTakeoff.ts` (`measureTrayRunsFromVectors`) | **Critical** |
| C2 | **Fittings estimated from annotation direction changes** — T-pieces, cross-pieces, 90° bends are guessed from annotation position shifts, not actual path intersections. Once vector geometry is available (C1), fittings can be detected from line endpoint coincidence. | `server/services/containmentTakeoff.ts` fitting detection section | High |
...
*Single source of truth for all Claude sessions on IdoYourQuotes. Update this file whenever a flow changes, a bug is fixed, or a feature is added. Version: March 2026 — updated 17 Mar 2026.*

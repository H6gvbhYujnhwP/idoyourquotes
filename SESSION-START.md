...
This block captures the complete plan agreed with Wez on 17 Apr 2026. Deliveries 1–2 shipped 18 Apr. The next session picks up from item 3 and works top-to-bottom.
...
### Build Order — 6 Remaining Deliveries, One At A Time (Deliveries 1–2 shipped 18 Apr)
...
3. **Sector visibility filter.** In `client/src/lib/tradeSectors.ts`, add a `VISIBLE_SECTORS` constant containing the four GTM keys plus `custom`. Export a `getVisibleTradeSectorOptions(currentValue?: string)` helper that returns filtered `TRADE_SECTOR_OPTIONS` but preserves `currentValue` if it's a hidden sector (so existing users whose `defaultTradeSector` is e.g. `electrical` still see their current value in Settings and can keep it). Update `Register.tsx` and `Settings.tsx` to call the helper. Old sector keys remain valid at engine level — `engineRouter.selectEngine()` routes them correctly regardless of dropdown visibility — so legacy quotes and legacy users are unaffected.

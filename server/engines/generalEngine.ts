...
- materials: Every billable line item with catalog-matched prices where possible. Use the EXACT "item" name from the catalog. Use the EXACT "unit" from the catalog (Per Hour, Per Month, Per 5,000, Session, etc.).
  For "description" — apply these rules based on pricingType:
  • STANDARD items: Write a specific 1-sentence description drawn from the evidence explaining what this item delivers in context for THIS quote. E.g. "1 day workshop to configure VLAN segregation for 9 APs across 10 tenant offices" — not just "workshop labour". If the catalog has a description use it as a starting point but enrich it with scope-specific detail from the evidence.
  • MONTHLY or ANNUAL items (contracts, retainers, ongoing services): The description IS the sales document — the client decides whether to commit based on what they read. Write the description as a structured breakdown: start with a concise summary sentence, then list every element included using "• " bullet points. Draw from the evidence AND your knowledge of what a well-structured contract at this price point for this sector typically covers. Examples of what to include per sector:
    - IT/MSP contracts: monitoring coverage (devices, hours), incident response SLA, included remote support hours, patch management, backup verification, reporting cadence, escalation path
    - Cleaning contracts: visit frequency, areas covered, cleaning tasks per visit, consumables included, supervisor checks, emergency call-out terms
    - Maintenance/FM contracts: planned maintenance visits per year, reactive call-out SLA, included labour hours, parts coverage, compliance documentation
    - Pest control contracts: inspection frequency, covered pests, treatment methods, documentation/certificates provided
    Never write just a single sentence for a monthly or annual item. Minimum 4 bullet points.

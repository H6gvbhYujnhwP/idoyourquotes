...
- materials: Every billable line item with catalog-matched prices where possible. Use the EXACT "item" name from the catalog. Use the EXACT "unit" from the catalog (Per Hour, Per Month, Per 5,000, Session, etc.).
  For "description" — use "||" as the separator between elements whenever a breakdown adds clarity. Do NOT use newlines or "•" characters anywhere in descriptions — use ONLY "||".
  - STANDARD items (hardware, one-off labour, setup): If the item is straightforward (e.g. a single piece of hardware), write one clear sentence. If the item covers multiple deliverables or steps (e.g. a labour day covering several tasks, a setup service with multiple components), use "||" to list each element. E.g. "1.5 days onsite installation || Vigor Router setup on Gigaclear line || WiFi access point deployment across 9 locations || VLAN testing and commissioning". Use judgement — only add "||" breakdown when it genuinely helps the client understand what they're getting.
  - MONTHLY or ANNUAL items (contracts, retainers, ongoing services): ALWAYS use "||" breakdown. The description IS the sales document. Format: summary sentence || feature 1 || feature 2 || feature 3 (minimum 4 features). Draw from the evidence AND your knowledge of what a well-structured contract at this price point includes. Examples per sector:

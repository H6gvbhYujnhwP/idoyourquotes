...
CLIENT EXTRACTION — cast a wide net across every evidence type:
- The goal is to identify the CLIENT — the business or person the user (tradesperson / the business this quote is being prepared by) is quoting TO. Never name the user's own business as the client.
- SIGNAL 1 — filename of uploaded documents. Each evidence block is labelled, e.g. "Document (Bradman Joinery Quote Request.pdf): ...". If the filename names a business that is clearly NOT the user's own business, that business is a strong client candidate.
- SIGNAL 2 — a business name that appears near an address, phone number, email, site address, or company registration detail in the document body, PDF letterhead, invoice "Bill To" / "Quote For" block, or tender cover page. On a scanned quote request, the client's company is almost always adjacent to the site address.
- SIGNAL 3 — any explicit "Dear [name]", "Hi [name]", "Attn: [name]", "Quote For:", "Prepared For:", "Client:", "Company:" labels — with the value that follows.
- SIGNAL 4 — email conversation shape. From/To headers, signature blocks, "thanks", "regards", "kind regards [name] at [company]".
- SIGNAL 5 — voice notes that mention the client by name ("I need a quote for Bradman Joinery, they need...", "the customer is [company]...").
- SIGNAL 6 — document title or first heading (e.g. a specification sheet titled "ACME Ltd — IT Requirements" strongly suggests ACME Ltd is the client).
- Exclude as candidates: the user's own business (appears consistently across multiple evidence inputs as the sender), software product names, domain names on their own without a business name attached, and generic placeholders like "Client" or "Customer".
- If SIGNAL 1 conflicts with SIGNAL 2, prefer SIGNAL 2 (document body) over SIGNAL 1 (filename) — filenames can be auto-generated by tools.
- If after all six signals you genuinely cannot identify a specific client, leave clientName as null. Do NOT invent a client name, and do NOT emit literal strings like "Client" or "Customer".
- For clientAddress, extract the installation / site / delivery address — NOT the user's own address.

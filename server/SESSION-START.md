...
| `userPrompt` | `quotes` DB column + useState | Textarea onBlur (electrical only) | `hasSavedQDS` guard (Case 2, legacy), `TakeoffPanel` symbol filtering (electrical only) |
...
| Takeoff Instructions (electrical) | `quotes.user_prompt` | Textarea onBlur (electrical sector only) | `TakeoffPanel` symbol filtering, `hasSavedQDS` guard (legacy fallback) |
...
- `userPrompt` DB column is now ONLY for electrical sector takeoff symbol filtering — never carries QDS data
...
| 20 Mar 2026 | `server/services/stripe.ts` | **customer.subscription.deleted webhook guard — root cause of solo regression.** When upgrading via createCheckout (new subscriber path), Stripe cancels the old subscription and creates a new one. The `customer.subscription.deleted` webhook for the old subscription was firing AFTER `activateSubscription` had already written the new subscription ID to the DB. The unconditional reset wiped `subscriptionTier=pro` back to `subscriptionTier=trial`. Fix: fetch the org's current `stripeSubscriptionId` before resetting. If the deleted subscription ID does not match the org's current one, log and return — the org has already moved on. Only resets to trial when the deleted sub IS the current one (genuine cancellation). This guard must never be removed — it protects all future upgrade flows that go through createCheckout. |

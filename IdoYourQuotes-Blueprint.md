...
**Status:** MVP Complete, Multi-Tenancy Complete, Production Live, Pre-launch Hardening — P0 Security & Cost Shipped, P1 Customer-facing Bugs Shipped, Anti-gaming Downgrade Shipped, P2 Hygiene Pending
...
- [ ] **Fix the adjacent phone-bug siblings on the PDF generator.** Folder: server, file: pdfGenerator.ts, lines 1542 and 1543. Same wrong-field-name pattern as the bug 4 fix in E.17 — `organization?.address` and `organization?.email` should be `organization?.companyAddress` and `organization?.companyEmail`. Two more pre-existing TS errors in the baseline. Best done as part of the broader electrical-removal cleanup, since the function they live in (`generateElectricalQuoteHTML` at line 1532, plus its call site at line 611) is dead code for new quotes — the cleanest fix is to delete the whole function rather than patching individual lines. Would also drop the file from the locked list since it would no longer carry electrical-trade-specific code.

...
- [x] **5. Chapter sets for Cleaning, Pest Control and Web & Marketing, plus the IT content fixes.** ✅ SHIPPED in Delivery 2.14 (Sep 17, 2026). Three sector sets built on a shared eleven-chapter spine; IT's set kept its nineteen chapters and gained the content fixes. Chapter counts: IT 19, Cleaning 22, Pest Control 20, Web & Marketing 18.
...
| 3.30 | Sep 17, 2026 | Delivery 2.14 Chunk 5 — sector chapter sets. The first chunk of the programme a customer would notice, and the last large one. RESEARCH FIRST, and it changed the design: commercial cleaning and pest control are not sold with sales proposals, they are TENDERED. Councils, housing associations and schools publish the sections a bidder must answer and score each one separately, so a section not written scores zero. Live Find a Tender notices (Sep 2026) give the structure — supplier information, service delivery methodology, staffing model and supervision, recruitment/vetting/induction/training, business continuity including sickness and absence cover, quality assurance and contract management — and the BPCA's contract-specification guidance gives pest control's. The first draft of these lists, written from intuition, missed four SCORED cleaning sections outright (TUPE, social value, environmental and waste management, business continuity and absence cover) and missed Integrated Pest Management entirely, which is the organising idea of the whole pest discipline. Web and digital marketing is the opposite animal: a persuasive sales document with no imposed structure, where the recurring failure is scope ambiguity rather than a missing section, so its middle is built around the arguments that actually happen — deliverables, revision rounds, who supplies content, whether ad spend is a fee, who owns what at the end. ARCHITECTURE: new folder `server/engines/chapterSets` — spine.ts (SlotDef, TABLE_GUIDANCE, the eleven shared chapters, the composer, and the house rules), commercialCleaning.ts, pestControl.ts, websiteMarketing.ts, index.ts. Every sector shares seven opening and four closing chapters and differs only in the middle, so the documents stay recognisably one product and a fifth sector is mostly a matter of writing its middle. slotIndex is DERIVED by the composer rather than written down, so inserting a chapter cannot silently renumber another set. SlotDef moved from the engine into spine.ts because the engine registers the sets and the sets would otherwise import the engine — a cycle; the engine re-exports it so no existing importer changed. Ids carry a version (`commercial-cleaning-v1`): a revised set is a NEW id, never an edit, because proposals already sent resolve against the id stamped into their saved state by Chunk 4. Folder `server`, file: sectorPacks.ts — each go-to-market sector now points at its own set; the other twenty-two still borrow IT's, which is what DEFAULT_CHAPTER_SET_ID now genuinely means. sectorCompleteness moves all four sectors from 4/6 to 5/6, outstanding: a contract starting point (Chunks 7 and 8). CONTENT FIXES, folded in as agreed: Key Personnel's instruction rewritten from a conditional ('if the brochure names no one…') to a positive one, because the model read the absence as reportable and printed 'The brochure does not name specific individuals' to a client; Cybersecurity & Compliance and Disaster Recovery made CONDITIONAL, having each carried a full page in a live proposal whose entire content was an explanation that the tender had not asked for them; and the house rules — never name the supplier's source documents to the reader, never write the quote reference into prose — applied to EVERY generated chapter programmatically rather than written into nineteen guidance strings, because a rule remembered per chapter is a rule that gets missed: the reference leaked from Contract Terms in one proposal and Pricing Summary in another. Tables permitted on five named chapters across the four sets. TS: exactly 69, per-file distribution byte-identical to baseline (diffed). Production build verified. PROOF, folder `scripts`, file: delivery-2-14-chunk5-proof.ts — 74 assertions: each sector resolves to its own set and non-GTM sectors still borrow the default; every set is well formed (unique ids, indices 1..n in order, exactly one pricing chapter, opens with the cover, closes with the call to action); every set shares the same spine; each middle carries the chapters the research said it must; every conditional chapter also carries the do-not-argue rule; no generated chapter lacks the house rules; and tables are permitted only where named. TWO EARLIER PROOFS NEEDED CORRECTING, both instructive. Chunk 3's asserted 'every sector points at the same set', true when it shipped and now true only of the twenty-two without their own — narrowed. Chunk 4's registered its SIMULATED set under the id `commercial-cleaning-v1`, which Chunk 5 made REAL: the proof was overwriting the live cleaning set and deleting it at the end, while its own count assertions masked the collision. Now uses a reserved id no sector can take, and counts relatively. All eight proofs in the 2.14 series pass. |

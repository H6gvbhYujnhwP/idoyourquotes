...
**Status:** MVP Complete, Multi-Tenancy Complete, Production Live, Pre-launch Hardening — P0 Security & Cost Shipped, P1 Customer-facing Bugs Shipped, Anti-gaming Downgrade Shipped, PDF Org-header Field-name Cleanup Shipped, Brand Extraction Polling Shipped, Customer Email Coverage Pending, P2 Hygiene Mostly Pending
...
**Status (May 7, 2026):** All three P0 / P1 customer-impacting categories — App Security (E.15), Cost Protection (E.16), and Customer-facing Bugs (E.17) — are now shipped. E.18 changed the anti-gaming behaviour from "block second signup" to "skip free trial on second signup" and fixed the silent first-time trial bug. Remaining work is P2 code hygiene plus the post-launch bot polish list.
...
- A new environment variable, `ANTI_GAMING_BYPASS_DOMAINS`, holds a comma-separated list of domains exempt from the anti-gaming check. Domains in this list always get a fresh 14-day trial regardless of history. Set this in Render's environment for any domain the platform owner uses for testing the signup / trial flow (e.g. `thegreenagents.co.uk`). Empty / unset means no bypass.

#### Customer email coverage (P1) — Pending

Discovered May 7, 2026 while diagnosing why a real Pro signup received a "your 14-day trial is active" email instead of a Pro confirmation. Two related gaps in the customer email flow:

- [ ] **No subscription confirmation email after first paid checkout.** Folder: server/services. The Stripe webhook handlers (`checkout.session.completed`, `customer.subscription.created`, `customer.subscription.updated`) update the database with the new tier, status, and billing period, but never trigger any email to the customer. So when a user takes the trial offer at register, then immediately pays for Pro/Solo/Team, they get the trial welcome email at email-verification time but no confirmation that their payment succeeded and their subscription is active. The existing `sendTierChangeEmail` only fires for paid-tier-to-paid-tier upgrades inside the app (e.g. Solo → Pro), wired into `subscriptionRouter`'s upgrade flow — it doesn't cover the trial-to-first-paid transition that goes through Stripe's checkout webhook. Fix: add a new `sendSubscriptionActivatedEmail` template in emailService and wire it into the `checkout.session.completed` webhook handler in stripe.ts. Email should include tier name, monthly cost, what's unlocked (quote allowance, team seats, AI credits), next billing date, and a link to manage the subscription.
- [ ] **Trial welcome email always claims "14-day trial active" regardless of state.** Folder: server/services. The `sendWelcomeEmail` template fires unconditionally at email verification with the subject "Welcome to IdoYourQuotes — your 14-day trial is active!" and body "Your email is verified and your 14-day free trial is now active." This is a lie for any user who came in via the E.18 "domain previously used → no trial" path. They got registered with no trial, redirected to /pricing?trial=skipped, but if they verify their email later, they still receive this email claiming an active trial. Fix: at email-verification time (in oauth.ts), look up the user's primary org, check whether `trialEndsAt` is in the future. If yes, send the existing trial-welcome email. If no (skipped trial, or subscription already active), send a different state-aware variant that says "Your account is verified — choose a plan below to get started" or similar.

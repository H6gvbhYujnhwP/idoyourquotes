...
| Starter catalogue (items, with buy-in costs) | 88 | 43 | 25 | 23 |
...
- [ ] **3. Sector pack registry.** One definition per sector holding all seven artefacts, replacing five registries keyed three different ways. Initially holds exactly what exists today, with the IT chapter set lifted in verbatim and the other three pointing at it. No behaviour change.
- [ ] **4. Stamping and legacy handling.** Saved proposals record which chapter set and version produced them; unstamped states read as the frozen legacy IT set. Proves out by regenerating a chapter on the existing Sorrells proposal after the sets have diverged.
- [ ] **5. Chapter sets for Cleaning, Pest Control and Web & Marketing.** The first chunk a customer would notice. Sets need not be the same length. **Fold the outstanding generated-content faults into this chunk** — they live in the same guidance strings: Key Personnel telling the client that the brochure names nobody, the raw quote reference leaking into Contract Terms, and the invented service start date contradicting the Acceptance page.

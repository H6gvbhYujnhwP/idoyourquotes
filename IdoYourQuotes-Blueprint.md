...
| 3.11 | Sep 11, 2026 | VAT fix — delivery 1 of the Xero repeating-invoices sequence (2 = contracts made per-business, 3 = Xero). Root cause: Settings pre-filled the VAT box with 20 when nothing had ever been saved, so organisations that never pressed Save had no stored rate while the screen claimed 20%; their new quotes silently got 0%. Render check on 11 Sep found orgs 3, 4 and 13 with no stored rate and 50 zero-VAT quotes across all five (all test orgs, no real customers). 0% was also read four different ways: the colour-template proposal promoted it to 20%, the brochure proposal treated it as no VAT, the contract printed 'including VAT at 0%', the terms clause said 'No VAT is applicable'. Fix: new single helper (folder server/services, file: vatRate.ts) defining the rule — organisation rate lives in defaultDayWorkRates.defaultVatRate, 0 = not VAT registered, absent reads as 20; a quote's 0% means no VAT applicable and is never promoted. Folder server, file: db.ts — createOrganization stamps defaultVatRate 20 on every new org; createQuote takes the org's rate whenever the caller supplies none (covers the demo-quote seeder), an explicit caller rate including 0 always respected. Folder server/services, file: slotContentBuilder.ts — pricing table no longer turns 0 into 20; at 0 it keeps its four columns but shows dashes and a 'No VAT applicable' footer. Folder server, file: docxGenerator.ts — Word export prints 'No VAT applicable' instead of 'VAT (0%): £0.00'. Folder client/src/pages, file: Settings.tsx — VAT number box replaced by an explicit 'VAT registered' / 'Not VAT registered' choice, rate field shown only when registered, validated before save. Data: repo root, file: migration-vat-backfill.sql — idempotent, single transaction; orgs with no stored rate get 20 (other keys in the JSON preserved, explicit 0 untouched); zero-VAT quotes in registered orgs get the org rate with tax_amount and total recalculated as recalculateQuoteTotals does. Xero note: the integration will send ex-VAT line prices and let Xero calculate VAT; IDYQ's VAT figure is never sent. Not touched: routers.ts, QuoteWorkspace.tsx, AdminPanel.tsx, pdfGenerator.ts (locked — its original quote PDF still prints a 'VAT (0%)' row at 0% and rounds the label to whole numbers; awaiting permission). Noted, not fixed: docx totals sum all line items (monthly and annual included) before adding VAT; legacy modern/bold/structured templates label the one-off total 'inc. VAT' even at 0%; contract '?? 20' fallback in brandedProposalRouter.ts is dead code — contract VAT wording is handled in delivery 2. Also found: contract_documents has zero rows for every org, confirming the contract-documents sub-router has never been mounted live (apply-contractdocument-router.mjs unapplied in the repo); delivery 2 mounts it after restricting Gold/Silver seeding to Sweetbyte. TS: the zip measures 81 because of that missing mount (12 errors, all 'contractDocument does not exist' in ContractDocumentsTab.tsx and BrandedProposalWorkspace.tsx); this delivery holds 81 with per-file distribution and messages identical, and with the mount applied on a scratch copy it measures exactly 69, identical to the pre-delivery 69 distribution. Production build verified. |

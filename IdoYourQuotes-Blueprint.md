...
- [ ] **5. Chapter sets for Cleaning, Pest Control and Web & Marketing.** **Open question added 17 Sep 2026:** should chapter bodies gain real table support? Several chapters read better as two-column mappings, and 4a resolved a live fault by banning tables rather than drawing them. Decide before writing the new sets, since it changes what their guidance can ask for. The first chunk a customer would notice. Sets need not be the same length. **Fold the outstanding generated-content faults into this chunk** — they live in the same guidance strings: Key Personnel telling the client that the brochure names nobody, the raw quote reference leaking into Contract Terms, and the invented service start date contradicting the Acceptance page.
